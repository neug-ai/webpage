...
- **原生图数据管理** — 以持久化存储方式保存实体、关系及属性，并支持显式事务、检查点（checkpoints）和预写日志（write-ahead logging）。
...
这三种访问方式均作用于同一底层数据。插入、更新与删除操作会原子性地维护图属性及其对应的向量或全文索引。已提交的索引状态将通过检查点与预写日志随图数据一同持久化和恢复。
