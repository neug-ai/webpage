...
本教程展示如何使用图算法来**构建和维护代码 Wiki**，
结合 NeuG（版本 >= v0.1.3）及其 GDS（图数据科学）扩展：

1. **构建** — 导入文件级关系图，使用 Leiden 检测 Wiki 章节
   边界，并使用 PageRank 挑选每个章节的核心文件。
2. **更新** — 增量添加新文件，使用冻结分配的 Leiden 重新聚类
   （现有章节保持稳定），并使用 Cypher 计算章节级增量。

运行示例是从 NeuG 代码库中提取的文件级关系图：
每个节点是一个源文件，每条边表示“文件 A 与文件 B 相关”。这些
关系派生自底层的代码图 — 例如，如果
文件 A 中的函数调用了文件 B 中的函数，那么 A 和 B 就会通过一条边连接。

**时间**：约 15 分钟
**先决条件**：`pip install neug>=0.1.3`
