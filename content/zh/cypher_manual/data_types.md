...
下表展示了 NeuG 支持的所有数据类型及其与 Neo4j 的差异。`System Default Value`（系统默认值）列表示用户未在模式中显式定义默认值，且原始数据中未提供相应数据字段（或提供为 `Null` 值）时，系统在数据导入过程中自动分配的值。此机制可避免产生 `Null` 值，确保数据一致性，并为后续查询和计算提供稳定的默认值。
