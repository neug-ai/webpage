...
export const CustomNavbar: React.FC<NavbarProps> = ({
  ...
  // 获取文档链接 - 根据当前 URL 判断 basePath
  const [documentLink, setDocumentLink] = useState("/en/overview/introduction/");
  
  useEffect(() => {
    const userLang = getUserLanguage();
    // 根据当前 URL 判断是否需要 /neug 前缀
    const pathname = window.location.pathname;
    const basePath = pathname.startsWith("/neug") ? "/neug" : "";
    setDocumentLink(`${basePath}/${userLang}/overview/introduction/`);
  }, []);
